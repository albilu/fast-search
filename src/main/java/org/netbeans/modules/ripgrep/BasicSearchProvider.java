...
import org.netbeans.modules.ripgrep.IgnoreListPanel.IgnoreListManager;
 ...
 * @author jhavlin
 */
@ServiceProvider(service = SearchProvider.class, position = 1) //@ServiceProvider(service = BasicSearchProvider.class, position = 1)
...
//                    basicSearchCriteria.getSearchPattern());
            ...
            SearchPattern.MatchType matchType = basicSearchCriteria.getMatchType();//Literal/Regex...
            String fileNamePatternExpr = basicSearchCriteria.getFileNamePatternExpr();//File Name Patterns str
            RipGrepMatcher am = new RipGrepMatcher(basicSearchCriteria.getSearchPattern())
                    .setSearchTerm(basicSearchCriteria.getTextPatternExpr())
                    .setCaseSensitive(basicSearchCriteria.isCaseSensitive())
                    .setWholeWord(basicSearchCriteria.isWholeWords())
                    .setSearchInArchives(basicSearchCriteria.isSearchInArchives())
                    .addScopeFiles(ssi.getSearchRoots().stream().map((t)
                            -> t.getFileObject().getPath()).collect(Collectors.toList()))
                    .setUseRegex(matchType.equals(SearchPattern.MatchType.REGEXP))
                    .setUsePcre(matchType.equals(SearchPattern.MatchType.BASIC))
                    .setLiteral(matchType.equals(SearchPattern.MatchType.LITERAL))
                    .setFileNamePattern(fileNamePatternExpr)
                    .setGlobeInclude(false)
                    .setSearchInGeneratedSources(basicSearchCriteria.isSearchInGenerated())
                    .setUseIgnoreList(false)
                    .addIncludes(Arrays.asList(fileNamePatternExpr.split(",")));
        ...
        public boolean isUsable(NotificationLineSupport notifySupport) {
            ...
            if (!usable /*ripgrep ovveride search pattern regex check*/ && !bsc.isTextPatternInvalid()) {
                String msg;
            ...
            return bsc.isTextPatternInvalid() /*ripgrep ovveride search pattern regex check*/
                    || usable;
        }
    ...
    public static void startSearch(
        ...
        SearchInfo searchInfo = bestScope.getSearchInfo();
        SearchPattern.MatchType matchType = criteria.getMatchType();//Literal/Regex...
        String fileNamePatternExpr = criteria.getFileNamePatternExpr();//File Name Patterns str

        BasicComposition composition = new BasicComposition(
                searchInfo, new /*DefaultMatcher*/ RipGrepMatcher(searchPattern)
                        .setSearchTerm(criteria.getTextPatternExpr())
                        .setCaseSensitive(criteria.isCaseSensitive())
                        .setWholeWord(criteria.isWholeWords())
                        .setSearchInArchives(criteria.isSearchInArchives())
                        .addScopeFiles(searchInfo.getSearchRoots().stream().map((t)
                                -> t.getFileObject().getPath()).collect(Collectors.toList()))
                        .setUseRegex(matchType.equals(SearchPattern.MatchType.REGEXP))
                        .setUsePcre(matchType.equals(SearchPattern.MatchType.BASIC))
                        .setLiteral(matchType.equals(SearchPattern.MatchType.LITERAL))
                        .setFileNamePattern(fileNamePatternExpr)
                        .setGlobeInclude(false)
                        .setSearchInGeneratedSources(criteria.isSearchInGenerated())
                        .setUseIgnoreList(false)
                        .addIncludes(Arrays.asList(fileNamePatternExpr.split(","))),
                criteria, null);
